...
public:
  ...
  bool allEqual() const {
    int firstValue = buffer_[head_];
    for (int i = 1; i < BUFFER_SIZE; ++i) {
      if (buffer_[(head_ + i) % BUFFER_SIZE] != firstValue) {
        return false;
      }
    }
    return buffer_[(head_ + BUFFER_SIZE - 1) % BUFFER_SIZE] != -1;
  }
...
void setup()
{
    ...
    BLE.setLocalName("Nano 33 BLE (Detection)"); 
    BLE.advertise();
...
byte mapHandToByte(const char *hand) {
  byte value;

  if (strcmp(hand, "uncertain") == 0) {
    value = 0;
  } else if(strcmp(hand, "nothing") == 0) {
    value = 1;
  } else if(strcmp(hand, "rock") == 0) {
    value = 2;
  } else if(strcmp(hand, "paper") == 0) {
    value = 3;
  } else if(strcmp(hand, "scissors") == 0) {
    value = 4;
  } else {
    value = 255;
  }
...
void loop()
{
    ...
    while(stop_inferencing == false) {
...
#else
        const float UNCERTAIN_BORDER = 0.5;
        const char *highest_label = "uncertain";
        const char *last_highest_label = "";
        float highest_value = 0;
        
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
            if (result.classification[ix].value > highest_value && result.classification[ix].value >= UNCERTAIN_BORDER) {
        ...
        ei_printf("Highest value: %s - %.5f\n", highest_label,
                                                highest_value);

        buffer.push(mapHandToByte(highest_label));

        if (buffer.allEqual()) {
          BLEDevice peripheral = BLE.available();
          ei_printf("%d times same gesture -> Trying to send over BLE: %s \n", BUFFER_SIZE, highest_label);
          buffer.clear();
          if (peripheral.connected()) {
            gestureCharacteristic.writeValue(mapHandToByte(highest_label));
            ei_printf("Sent\n");
          } else {
            ei_printf("Not connected to peripheral anymore. Trying to connect again...\n"); 
              while(!connectToPeripheral());
              gestureCharacteristic.writeValue(mapHandToByte(highest_label));
              ei_printf("Sent\n");
          }
        }
